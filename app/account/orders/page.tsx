...
import { MapPin, Package, Phone, ReceiptText } from "lucide-react";
  ...
  useEffect(() => {
    const loadOrders = async (user: User) => {
      const { data } = await supabase
        .from("orders")
        .select("id,order_number,status,payment_status,total_amount,customer_name,customer_phone,address_line1,address_line2,city,state,pincode,created_at,order_items(id,product_name,quantity,unit_price,line_total,image_url)")
        .eq("user_id", user.id)
